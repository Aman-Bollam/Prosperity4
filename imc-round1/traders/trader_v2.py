...
class Trader:
    ...
    def _trade_product(
        ...
        if pos > 0:
            skew = 1 + int((pos / limit) * 5)
            bid_quote -= skew
            ask_quote -= max(1, (skew + 1) // 2)
        elif pos < 0:
            skew = 1 + int((-pos / limit) * 5)
            bid_quote += max(1, (skew + 1) // 2)
            ask_quote += skew
    ...
    def _fair_value(self, product: str, mids: List[float]) -> float:
        ...
        return 0.55 * 10000.0 + 0.45 * local
    ...
    def _quote_sizes(self, product: str, pos: int, limit: int, buy_cap: int, sell_cap: int):
        ...
        if pos >= hard:
            buy_sz = 0
            sell_sz = min(max(base + 8, sell_sz), sell_cap)
        elif pos >= soft:
            buy_sz = min(buy_sz, max(0, buy_cap // 6))
            sell_sz = min(max(base + 4, sell_sz), sell_cap)

        if pos <= -hard:
            sell_sz = 0
            buy_sz = min(max(base + 8, buy_sz), buy_cap)
        elif pos <= -soft:
            sell_sz = min(sell_sz, max(0, sell_cap // 6))
            buy_sz = min(max(base + 4, buy_sz), buy_cap)
