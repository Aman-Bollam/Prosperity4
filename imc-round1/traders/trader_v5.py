...
class Trader:
    ...
    def pepper_signal(self, state: TradingState, order_depth: OrderDepth):
        ...
        # FIX: timestamps go 0..999900, use 1000000 so bias decays 28→0 over full day
        time_bias = int(28 * (1.0 - state.timestamp / 1000000.0))
    ...
    def ash_signal(self, order_depth: OrderDepth):
        ...
        target = self.clamp(int(round(-1.7 * dev + 8.0 * imb1 + 2.5 * edge)), -55, 55)
    ...
    def trade_ash(self, order_depth: OrderDepth, position: int) -> List[Order]:
        ...
        buy_threshold = 1.20 if edge > 1.0 else 1.60
        sell_threshold = 1.20 if edge < -1.0 else 1.60
