...
class Trader:
    ...
    @staticmethod
    def level_volume(book: Dict[int, int], price: Optional[int], is_sell: bool = False) -> int:
        if price is None:
            return 0
    ...
    def pepper_signal(self, state: TradingState, order_depth: OrderDepth):
        ...
        if self.pepper_anchor is None:
            ...
            # Hidden days can shift the anchor a bit. Keep it adaptive but not noisy.
        ...
        # Blend observed mid and deterministic trend; trend gets more weight when the book is thin.
        fair = 0.55 * trend_fair + 0.45 * mid + edge

        # Pepper has a strong structural upward drift in the training data.
    ...
    def ash_signal(self, order_depth: OrderDepth):
        ...
        # Mostly mean reversion around 10000, but allow short-term order-flow tilt.
        target = self.clamp(int(round(-1.4 * dev + 8.0 * imb1 + 2.5 * edge)), -55, 55)
    ...
    def market_make(
        ...
        # Directional skew toward the target inventory.
        if target > position:
        ...
        # Strong inventory protection near the limits.
        if position > 60:
